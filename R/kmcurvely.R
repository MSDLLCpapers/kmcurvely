...
kmcurvely <- function(meta = meta_tte_example(),
  ...
  tbl_surv <- tbl_surv %>%
    ...
    dplyr::mutate(surv = round(surv, 2),
  ...
  for (ept in endpoint) {
    for (subgrp in c("all", subgroup)) {
      ...
      tbl_at_risk_new <- tibble::as_tibble(tbl_at_risk_new) %>%
        dplyr::mutate(group = row.names(tbl_at_risk_new),
  ...
  km_plot <- ggplot2::ggplot() +
    ggplot2::geom_step(data = surv_shared,
  ...
  select_endpoint <- crosstalk::filter_select(id = param_random_id, sharedData = surv_shared,
  ...
  select_subgroup <- crosstalk::filter_select(id = subgroup_random_id, sharedData = surv_shared,
                                                multiple = FALSE, group = ~subgroup, label = "Subgroup")
  # ----------------------------------------------- #
  #    combine everything
  # ----------------------------------------------- #

  plot <- crosstalk::bscols(widths = c(3, 9),
